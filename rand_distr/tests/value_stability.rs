...
fn binominal_stability() {
    ...
    test_samples(353, Binomial::new(2000, 0.6).unwrap(), &[
        1194, 1208, 1192, 1210,
    ]);
}
...
fn geometric_stability() {
    ...
    test_samples(464, Geometric::new(0.05).unwrap(), &[
        24, 51, 81, 67, 27, 11, 7, 6,
    ]);
    test_samples(464, Geometric::new(0.95).unwrap(), &[
        0, 0, 0, 0, 1, 0, 0, 0,
    ]);
...
fn hypergeometric_stability() {
    ...
    test_samples(7221, Hypergeometric::new(99, 33, 8).unwrap(), &[
        4, 3, 2, 2, 3, 2, 3, 1,
    ]); // Algorithm HIN
    test_samples(7221, Hypergeometric::new(100, 50, 50).unwrap(), &[
...
fn poisson_stability() {
    ...
    test_samples(223, Poisson::new(27.0).unwrap(), &[
...
fn dirichlet_stability() {
    ...
    assert_eq!(rng.sample(Dirichlet::new(&[1.0, 2.0, 3.0]).unwrap()), vec![
        0.12941567177708177,
        0.4702121891675036,
        0.4003721390554146
    ]);
