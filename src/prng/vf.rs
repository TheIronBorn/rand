...
macro_rules! make_vf {
    ($rng_name:ident, $version:ident, $vector:ident, $scalar:ty, $half:ident, $vec8:ident, $shuf:ident, $indices:expr, $rot:expr, $shr:expr, $shl:expr) => {
        ///
        #[derive(Debug)]
        pub struct $rng_name {
            a: $vector,
        ...
        impl RngCore for $rng_name {
            ...
            fn fill_bytes(&mut self, dest: &mut [u8]) {
                ...
                for _ in 0..dest.len() / CHUNK_SIZE {
                    let results = $vec8::from_bits(self.generate());
                    // results = results.to_le(); // TODO: look into reverse store intrinsics
                    results.write_to_slice_unaligned(&mut dest[read_len..]);
                    read_len += CHUNK_SIZE;
                }
                ...
                if remainder > 0 {
                    ...
                    #[repr(align(16))]
                    struct Aligned([u8; $vec8::lanes()]);
                    let mut buf = Aligned([0_u8; $vec8::lanes()]);
                    results.write_to_slice_unaligned(&mut buf.0);
    ...
    ( versions: $vector:ident, $scalar:ty, $half:ident, $vec8:ident, $shuf:ident, $indices:expr, $rot:expr, $shr:expr, $shl:expr,
        $name_a:ident, $name_b:ident, $name_c:ident, $name_d:ident, $name_e:ident, $name_f:ident, $name_g:ident,
    ) => {
        make_vf! { $name_a, vf_a, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_b, vf_b, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_c, vf_c, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_d, vf_d, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_e, vf_e, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_f, vf_f, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
        make_vf! { $name_g, vf_g, $vector, $scalar, $half, $vec8, $shuf, $indices, $rot, $shr, $shl }
    };

    ( 64bit: $vector:ident, $half:ident, $vec8:ident, $indices:expr, $shuf:ident, $($rng_name:ident,)+) => {
        make_vf! { versions: $vector, u64, $half, $vec8, $shuf, $indices, 29, 9, 3, $($rng_name,)+ }
    };
    ( 32bit: $vector:ident, $half:ident, $vec8:ident, $indices:expr, $shuf:ident, $($rng_name:ident,)+) => {
        make_vf! { versions: $vector, u32, $half, $vec8, $shuf, $indices, 13, 7, 3, $($rng_name,)+ }
    };
    ( 16bit: $vector:ident, $half:ident, $vec8:ident, $indices:expr, $shuf:ident, $($rng_name:ident,)+) => {
        make_vf! { versions: $vector, u16, $half, $vec8, $shuf, $indices, 7, 3, 2, $($rng_name,)+ }
    };
...
make_vf! { 64bit: u64x2, u32x4, u8x16, [1, 0, 3, 2], simd_shuffle4, VeryFast64x2a, VeryFast64x2b, VeryFast64x2c, VeryFast64x2d, VeryFast64x2e, VeryFast64x2f, VeryFast64x2g, }
make_vf! { 64bit: u64x4, u32x8, u8x32, [1, 0, 3, 2, 5, 4, 7, 6], simd_shuffle8, VeryFast64x4a, VeryFast64x4b, VeryFast64x4c, VeryFast64x4d, VeryFast64x4e, VeryFast64x4f, VeryFast64x4g, }
make_vf! { 64bit: u64x8, u32x16, u8x64, [1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14], simd_shuffle16, VeryFast64x8a, VeryFast64x8b, VeryFast64x8c, VeryFast64x8d, VeryFast64x8e, VeryFast64x8f, VeryFast64x8g, }

make_vf! { 32bit: u32x2, u16x4, u8x8, [1, 0, 3, 2], simd_shuffle4, VeryFast32x2a, VeryFast32x2b, VeryFast32x2c, VeryFast32x2d, VeryFast32x2e, VeryFast32x2f, VeryFast32x2g, }
make_vf! { 32bit: u32x4, u16x8, u8x16, [1, 0, 3, 2, 5, 4, 7, 6], simd_shuffle8, VeryFast32x4a, VeryFast32x4b, VeryFast32x4c, VeryFast32x4d, VeryFast32x4e, VeryFast32x4f, VeryFast32x4g, }
make_vf! { 32bit: u32x8, u16x16, u8x32, [1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14], simd_shuffle16, VeryFast32x8a, VeryFast32x8b, VeryFast32x8c, VeryFast32x8d, VeryFast32x8e, VeryFast32x8f, VeryFast32x8g, }
make_vf! { 32bit: u32x16, u16x32, u8x64, [1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14, 17, 16, 19, 18, 21, 20, 23, 22, 25, 24, 27, 26, 29, 28, 31, 30], simd_shuffle32, VeryFast32x16a, VeryFast32x16b, VeryFast32x16c, VeryFast32x16d, VeryFast32x16e, VeryFast32x16f, VeryFast32x16g, }
