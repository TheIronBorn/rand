...
use crate::distributions::utils::{
    BoolAsSIMD, FloatAsSIMD, FloatSIMDUtils, OverflowingAdd, SimdCombine, SimdSplit,
    WideningMultiply,
};
        ...
        impl UniformInt<$ty> {
                ...
                B2: SampleBorrow<$ty> + Sized,
            {
                ...
                if lo_order > range.wrapping_neg() {
                    ...
                    let (new_hi_order, _) =
                        (rng.gen::<u64>() as $u_extra_large).wmul(range as $u_extra_large);
    ...
    ($ty:ident, $unsigned:ident, $u_scalar:ident) => {
        ...
        impl UniformSampler for UniformInt<$ty> {
                      ...
                      B2: SampleBorrow<Self::X> + Sized
            {
                let low = *low_b.borrow();
                let high = *high_b.borrow();
                assert!(low.le(high).all(), "Uniform::new_inclusive called with `low > high`");
